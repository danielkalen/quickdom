...
var checkChildStructure, creator, elementSuffix, expect, i, j, len, len1, nonElementSuffix, ref, ref1, restartSandbox, sandbox, sandbox$, should,
  slice = [].slice;
...
suite("QuickDom", function() {
  ...
  suite("State", function() {
    test("States can be toggled on/off via .setState and polled via .getState", function() {
      var div;
      div = Dom.div();
      ...
      div.setState('$base');
      expect(div.getState('funny')).to.be["true"];
      return expect(div.getState('base')).to.be["false"];
    });
    test("If the value (2nd argument) is omitted from .setState, it'll default to 'true'", function() {
      var div;
      div = Dom.div();
      expect(div.getState('funny')).to.be["false"];
      div.setState('funny');
      expect(div.getState('funny')).to.be["true"];
      div.setState('funny');
      return expect(div.getState('funny')).to.be["true"];
    });
    test("All states can be cleared/toggled off via .resetState", function() {
      ...
      return expect(div.getState('relaxed')).to.be["false"];
    });
  ...
  suite("Manipulation", function() {
    ...
    test(".detach()", function() {
      ...
      expect(div.getState('happy')).to.be["true"];
      return expect(div.getState('relaxed')).to.be["true"];
    });
    ...
    test(".empty()", function() {
      ...
      expect(A.getState('happy')).to.be["true"];
      return expect(B.getState('happy')).to.be["true"];
    });
    test(".wrap()", function() {
      ...
      return expect(wrapC.getState('relaxed')).to.be["true"];
    });
    ...
    return test(".text() - textContent getter/setter", function() {
      var div;
  ...
  suite("Templates", function() {
    ...
    test("Templates can be created from QuickElement instances", function() {
      ...
      return expect(templateSpawn.getState('happy')).to.be["false"];
    });
  ...
  return suite("Misc", function() {
    test("Chaining", function() {
      ...
      div = Dom.div();
      chainResult = div.on('abc', function() {}).emit('abc').off('abc').off().setState('abc').resetState().style().css('width', 12).attr('test', 123).prop('anotherTest', 123).append().appendTo().prepend().prependTo().before().after().insertBefore().insertAfter().html('<span></span>').text('some text').detach().wrap(Dom.section()).unwrap().wrap(Dom.header()).replace().appendTo(sandbox).wrap(head = Dom.header());
    ...
    return test("Invalid Arguments", function() {
      ...
      expect(div.setState()).to.equal(void 0);
      expect(div.setState(null)).to.equal(void 0);
      expect(div.setState(123)).to.equal(void 0);
